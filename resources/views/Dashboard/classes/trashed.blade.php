									...
									<table class="table text-md-nowrap" id="example1">
										...
										<tbody>
										  ...
										  <td>{{$loop->iteration}}</td>
											<td>{{$trashed->grades_en }}</td>
											<td>{{$trashed->grades_ar }}</td>
											<td>{{$trashed->name_english }}</td>
											<td>{{$trashed->name_arabic }}</td>

		
												 <td>
												 <form method="POST"  action="{{route('classes.restore',$trashed->id)}}" class="mr-2">
