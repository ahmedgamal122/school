...
@section('title')
@lang('lang.List_specializations')
@endsection
...
@section('page-header')
        ...
        <div class="my-auto">
            <div class="d-flex">
				...
				@lang('lang.List_specializations') </span>
            </div>
        </div>
    </div>
						...
						<div class="card">
							<div class="card-header pb-0">
								<div class="d-flex justify-content-between">
									<a href=" {{route('Specilization.create')}}">
							<button type="button" class="btn btn-primary" >
	
							@lang('lang.Adds_specializations')
							   </button>
                                </a>  
								</div>
							</div>
							...
							<div class="card-body">
								<div class="table-responsive">
									<table class="table text-md-nowrap" id="example1">
										...
										@foreach($Specilization as $Specilization)

											<tr>
											<td>{{$loop->iteration}}</td>
											<td>{{$Specilization->name_english }}</td>
											<td>{{$Specilization->name_arabic }}</td>
											<td>
												<div class="d-flex justify-content-between">
									<a href=" {{route('Specilization.edit',$Specilization->id)}}">
								<button type="button" class="btn btn-info"  >

								@lang('lang.Edite')
							   </button>
							   </a>
							   <form method="POST"  action="{{route('Specilization.delete',$Specilization->id)}}" class="mr-2">
