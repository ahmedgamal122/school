							...
							<div class="card-body">
								<div class="table-responsive">
									<table class="table text-md-nowrap" id="example1">
										...
										@foreach($Governrate as $Governrate)

											<tr>
											<td>{{$loop->iteration}}</td>
											<td>{{$Governrate->name_english }}</td>
											<td>{{$Governrate->name_arabic }}</td>
											<td>
												<div class="d-flex justify-content-between">
									<a href=" {{route('governorates.edit',$Governrate->id)}}">
								<button type="button" class="btn btn-info"  >

								@lang('lang.Edite')
							   </button>
							   </a>
							   <form method="POST"  action="{{route('governorates.delete',$Governrate->id)}}" class="mr-2">
